...
class QueryImpl(object):
    ...
    This query implementation wraps a tests function which is run when the
...
class Query(QueryImpl):
    ...
    the document to tests as the first argument and return ``True`` or
    ...
    def _generate_test(self, test, hashval):
        """
        Generate a query based on a tests function.

        :param test: The tests the query executes.
    ...
    def matches(self, regex, flags=0):
        """
        Run a regex tests against a dict value (whole string has to match).
    ...
    def search(self, regex, flags=0):
        """
        Run a regex tests against a dict value (only substring string has to
        match).
    ...
    def test(self, func, *args):
        """
        Run a user-defined tests function against a dict value.

        >>> def test_func(val):
        ...     return val == 42
        ...
        >>> Query().f1.tests(test_func)

        :param func: The function to call, passing the dict as the first
                     argument
        :param args: Additional arguments to pass to the tests function
        """
        return self._generate_test(
            lambda value: func(value, *args),
            ('tests', self._path, func, args)
        )
