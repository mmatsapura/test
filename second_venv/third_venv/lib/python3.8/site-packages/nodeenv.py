...
function deactivate_node -d 'Exit nodeenv and return to normal environment.'
    ...
    if tests -n "$_OLD_NPM_CONFIG_PREFIX"
    ...
    if tests -n "$_OLD_NODE_FISH_PROMPT_OVERRIDE"
    ...
    if tests (count $argv) = 0 -o "$argv[1]" != "nondestructive"
        # Self destruct!
...
function freeze -d 'Show a list of installed packages - like `pip freeze`'
    ...
    if tests "$NPM_VER" = "0"
        ...
        if tests (count $argv) -gt 0 -a "$argv[1]" = "-l"
            set NPM_LS "npm ls"
            set -e argv[1]
        end
...
if tests -z "$NODE_VIRTUAL_ENV_DISABLE_PROMPT"
    ...
    function fish_prompt
        ...
        if tests -n "__NODE_VIRTUAL_PROMPT__"
